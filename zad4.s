...
_ZN4BaseC2Ev:
.LFB7:
	//konstrukcija objekta tipa Base
	.cfi_startproc
	endbr64
	push	rbp
	.cfi_def_cfa_offset 16
	...
	//podešavanje pokazivača na tablicu virtualnih funkcija - definicija
...
_ZN9CoolClassC2Ev:
.LFB9:
	//konstrukcija objekta tipa CoolClass
	.cfi_startproc
	endbr64
	push	rbp
	.cfi_def_cfa_offset 16
	...
	//poziv konstruktora nadređenog razreda
	call	_ZN4BaseC2Ev
	//uzimanje adrese tablice virtualnih funkcija
	lea	rdx, _ZTV9CoolClass[rip+16]
	//podešavanje vptra (pokazivača na tablicu virtualnih funkcija) - definicija
	mov	rax, QWORD PTR -8[rbp]
	mov	QWORD PTR [rax], rdx
	nop
	leave
...
.LFB4:
	...
	//poziv new CoolClass() - unutar njega se alocira
	//memorija na heapu te konstruktoru prosljeđuje pokazivač na nju,
	//koji se također i vraća (dinamička alokacija tijekom izvođenja),
	//dok se kod poc objekt stvara na stogu i ne alocira se
	//memorija na heapu (statička alokacija tijekom prevođenja)
	call	_Znwm@PLT
	mov	rbx, rax
	mov	rdi, rbx
	//poziv konstruktora razreda CoolClass (za *pb)
	call	_ZN9CoolClassC1Ev
	mov	QWORD PTR -32[rbp], rbx
	lea	rax, -36[rbp]

	//pozivi funkcije set
	//za poc je potrebno manje instrukcija jer se
	//ne treba pristupati tablici virtualnih funkcija

	//poc

	//stavljanje konstante 42 na stog
	mov	esi, 42
	//stavljanje varijable poc na stog
	mov rdi, rax
	//poziv funkcije set za PlainOldClass
	call	_ZN13PlainOldClass3setEi

	//pb

	//referenciranje pb - dobivamo *pb
	mov	rax, QWORD PTR -32[rbp]
	//referenciranje *pb - dobivamo **pb (zapravo vptr,
	//pokazivač na tablicu virtualnih funkcija)
	mov	rax, QWORD PTR [rax]
	mov	rdx, QWORD PTR [rax]

	//stavljanje pb na stog
	mov	rax, QWORD PTR -32[rbp]
	//stavljanje konstante 42 na stog
	mov	esi, 42
	mov	rdi, rax

	//poziv prve funkcije iz virtualne tablice objekta pb
	call	rdx
	mov	eax, 0
...
_ZTV9CoolClass:
	.quad	0
	.quad	_ZTI9CoolClass
	//tablica virtualnih funkcija i njezine funkcije - deklaracija
